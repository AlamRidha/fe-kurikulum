<template>
    <v-container fluid>
        <MenuTitle msg="Menu | Evaluasi" class="text-subtitle-1 font-weight-medium" />
        <v-data-table :headers="headers" :items="data_evaluasi" :items-per-page="5" class="elevation-5 mt-4">
            <template v-slot:item.no="{ index }">
                {{ index + 1 }} </template>

            <template v-slot:item.status_evaluasi="{ item }">
                <span variant="flat"
                    :class="item.status_evaluasi === 'Terlaksana' ? 'text-caption flat text-green-lighten-5 bg-green pa-2 rounded-pill' : 'text-caption flat text-red-lighten-5 bg-red pa-2 rounded-pill'">
                    {{
                            ...
                            <v-card-text>
                                <v-container class="py-4">
                                    <v-form ref="form">
                                        ...
                                        <v-select v-model="forms.status_evaluasi" :items="itemStatus"
                                            label="Status Evaluasi"></v-select>
                                        <v-textarea v-model="forms.masalah_evaluasi" label="Masalah Evaluasi"
                                ...
                                <v-container class="py-4">
                                    <v-form ref="form">
                                        ...
                                        <!-- <v-text-field v-model="formsEdit.status_evaluasi"
                                            label="Status Evaluasi"></v-text-field> -->
                                        <v-select v-model="formsEdit.status_evaluasi" :items="itemStatus"
                                            label="Status Evaluasi"></v-select>
                                        <v-textarea v-model="formsEdit.masalah_evaluasi" label="Masalah Evaluasi"
                                            class="rounded-xl" bg-color="grey-lighten-2"></v-textarea>
                                    </v-form>
                                </v-container>
                            </v-card-text>
...
const deleteItemConfirm = async () => {
    try {
        const response = await axios.delete(`http://localhost:3000/evaluasi/${deleteId.value}`)
...
const updateData = async () => {
    try {
        ...
        const response = await axios.put(`http://localhost:3000/evaluasi/${formsEdit.value.idEval}`, updateData)
        if (response.status === 200) {
