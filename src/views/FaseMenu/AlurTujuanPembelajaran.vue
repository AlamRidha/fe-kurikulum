<template>
  <v-container fluid>
    <MenuTitle
      msg="Menu | Kurikulum Alur Tujuan Pembelajaran"
      class="text-subtitle-1 font-weight-medium"
    />

    <v-data-table
      :headers="headers"
      :items="alur_tujuan_pembelajaran"
      :items-per-page="5"
        ...
        <v-toolbar flat>
              ...
              <v-card-text>
                <v-container class="py-4">
                  <v-form ref="form" v-model="isFormValid">
                    ...
                    <v-combobox
                      v-model="forms.alur_tujuan_pembelajaran"
                      :items="deskripsti_tp"
          ...
          <v-dialog v-model="dialogShow" max-width="900px">
            <v-card class="py-3 px-2">
              <v-card-title>
                <span class="text-h5"
                  >Detail Data Alur Tujuan Pembelajaran</span
                >
              </v-card-title>
              ...
              <v-card-text>
                <v-container class="py-4">
                  <v-row align="start" class="ms-2">
                    ...
                    <v-col cols="3">
                      <v-sheet> Alur Tujuan Pembelajaran </v-sheet>
                    </v-col>
                    <v-col cols="1">
                      <v-sheet class="pa-1"> : </v-sheet>
                    </v-col>
                    <v-col cols="8">
                      <v-sheet class="pa-1 text-justify">
                        <div
                          class="text-justify"
                          v-html="
                            formattedText(dataShow.alur_tujuan_pembelajaran)
                          "
                        ></div>
                      </v-sheet>
                    </v-col>
...
const loadData = async () => {
  try {
    const response = await axios.get(
      `http://localhost:3000/kurikulum/${idMp}/alur_tujuan_pembelajaran`
    );
...
const deleteItemConfirm = async () => {
  try {
    const response = await axios.delete(
      `http://localhost:3000/kurikulum/alur_tujuan_pembelajaran/${deleteId.value}`
    );
...
const editItem = async (id) => {
  ...
  try {
    const response = await axios.get(
      `http://localhost:3000/kurikulum/alur_tujuan_pembelajaran/${id}`
    );
    const data = response.data;
    console.log(data);
    formsEdit.value = {
...
const updateData = async () => {
  try {
    ...
    const response = await axios.put(
      `http://localhost:3000/kurikulum/alur_tujuan_pembelajaran/${formsEdit.value.idAtp}`,
      updateData
    );
    if (response.status === 200) {
...
const getTujuanPembelajaran = async () => {
  try {
    ...
    deskripsti_tp.value = tujuan_pembelajaran.value.map(
      (tp) => tp.tujuan_pembelajaran
    );
