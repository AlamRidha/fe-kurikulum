    ...
    >
                ...
                <v-container class="py-4">
                  <v-form ref="form" v-model="isFormValid">
                    <v-select
                      ...
                      :items="bab_modul"
                      label="Nama Bab"
                      :rules="textRules('Nama Bab')"
                    ></v-select>
                    <v-text-field
                      v-model="forms.jenisAsesmen"
                      label="Jenis Asesmen"
                      :rules="textRules('Jenis Asesmen')"
                    ></v-text-field>
                    <v-textarea
                      name="Bentuk Asesmen"
                      v-model="forms.bentukAsesmen"
                      label="Bentuk Asesmen"
                      bg-color="grey-lighten-2"
                      :rules="textRules('Bentuk Asesmen')"
                    ></v-textarea>
                    <v-textarea
                      name="keterangan"
            ...
            <v-card class="py-3 px-2">
              ...
              <v-card-text>
                <v-container class="py-4">
                  <v-form ref="form">
                    <v-select
                      v-model="formsEdit.namaBab"
                      :items="bab_modul"
                      label="Nama Bab"
                    ></v-select>
                    <v-text-field
                      v-model="formsEdit.jenisAsesmen"
                      label="Jenis Asesmen"
                    ></v-text-field>
                    <v-textarea
                      name="Bentuk Asesmen"
                      v-model="formsEdit.bentukAsesmen"
                      label="Bentuk Asesmen"
                      bg-color="grey-lighten-2"
                    ></v-textarea>
                    <v-textarea
                      name="keterangan"
                      v-model="formsEdit.keterangan"
                      label="Keterangan"
                      bg-color="grey-lighten-2"
                    ></v-textarea>
                  </v-form>
              ...
              <v-card-text>
                <v-container class="py-4">
                  <v-row align="start" class="ms-2">
                    ...
                    <!-- Bentuk Asesmen -->
                    <v-col cols="3">
                      <v-sheet> Bentuk Asesmen </v-sheet>
                    </v-col>
                    <v-col cols="1">
                      <v-sheet class="pa-1"> : </v-sheet>
                    </v-col>
                    ...
                    <v-col cols="8">
                      <v-sheet class="pa-1 text-justify">
                        <div
                          class="text-justify"
                          v-html="formattedText(dataShow.keterangan)"
                        ></div>
                      </v-sheet>
                    </v-col>
                  </v-row>
      ...
      <template v-slot:item.actions="{ item }">
        ...
        <v-btn
          ...
          color="error"
          class="mx-2"
          @click="deleteItem(item.idAsesmen)"
        ></v-btn>
      </template>
    </v-data-table>
    <!-- snackbar -->
...
const loadData = async () => {
  try {
    const response = await axios.get(
      `http://localhost:3000/kurikulum/${idMp}/asesmen/`
    );
...
const deleteItemConfirm = async () => {
  try {
    const response = await axios.delete(
      `http://localhost:3000/kurikulum/asesmen/${deleteId.value}`
    );
...
const editItem = async (id) => {
  ...
  try {
    const response = await axios.get(
      `http://localhost:3000/kurikulum/asesmen/${id}`
    );
    const data = response.data;
    console.log(data);
    formsEdit.value = {
...
const updateData = async () => {
  try {
    ...
    const response = await axios.put(
      `http://localhost:3000/kurikulum/asesmen/${formsEdit.value.idAsesmen}`,
      updateData
    );
    if (response.status === 200) {
