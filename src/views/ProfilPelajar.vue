<template>
    <v-container fluid>
        <MenuTitle msg="Menu | Profil Pelajar Pancasila" class="text-subtitle-1 font-weight-medium " />

        <v-data-table :headers="headers" :items="dataProfilPelajar" :items-per-page="6" class="elevation-5 mt-4"
            :search="searchData">
            ...
            <template v-slot:item.elemen="{ item }">
                ...
                <v-btn @click="showDetails(item)" prepend-icon="mdi mdi-eye-outline" variant="tonal" color="primary"
                    class="text-none">Lihat
                    Data</v-btn>
            </template>
                ...
                <v-toolbar flat>
                        ...
                        <v-card class="py-3 px-2">
                            ...
                            <v-card-text>
                                <v-container class="py-4">
                                    <v-form ref="form">
                                        <v-text-field v-model="forms.dimensi" label="Elemen"></v-text-field>
                                        <textarea cols="56" rows="10" class="border-sm elevation-2 pa-2"
                                            v-model="forms.elemen" label="Dimensi" placeholder="Dimensi"></textarea>
                                    </v-form>
                                </v-container>
                            </v-card-text>
                    ...
                    <v-dialog v-model="dialogEdit" max-width="600px">
                        <v-card class="py-3 px-2">
                            ...
                            <v-card-text>
                                <v-container class="py-4">
                                    <v-form ref="form">
                                        <v-text-field v-model="formsEdit.dimensi" label="Dimensi"></v-text-field>
                                        <textarea cols="56" rows="10" v-model="formsEdit.elemen"
                                            class="border-sm elevation-2 pa-2" label="Elemen"
                                            placeholder="Dimensi"></textarea>
                                    </v-form>
                                </v-container>
                            </v-card-text>
                    ...
                    <v-dialog v-model="dialogShow" max-width="600px">
                        <v-card class="py-3 px-2">
                            ...
                            <v-card-text>
                                <v-container class="py-4">
                                    <v-row align="start" class="ms-2">
                                        ...
                                        <v-col cols="8">
                                            <v-sheet class="pa-1 text-justify">
                                                <div class="text-justify" v-html="formattedText(detailItem.elemen)">
                                                </div>
                                            </v-sheet>
                                        </v-col>
                                    </v-row>
                            ...
                            <v-card-actions>
                                <v-spacer></v-spacer>
                                <v-btn color="primary" variant="text" @click="closeShow" elevation="4">
                                    Tutup
                                </v-btn>
                            </v-card-actions>
                        </v-card>
            ...
            <template v-slot:item.aksi="{ item }">
                <v-btn density="comfortable" icon="mdi mdi-pen" color="success" class="mx-2"
                    @click="editItem(item.idProfil)"></v-btn>
...
const save = async () => {
    try {
        ...
        const response = await axios.post("http://localhost:3000/profilpelajar", newData)
...
const editItem = async (id) => {
    ...
    try {
        const response = await axios.get(`http://localhost:3000/profilpelajar/${id}`)
        const data = response.data
...
const updateData = async () => {
    try {
        ...
        const response = await axios.put(`http://localhost:3000/profilpelajar/${formsEdit.value.idProfil}`, updateData)
        if (response.status === 200) {
            dialogEdit.value = false
            loadData()
...
const deleteItemConfirm = async () => {
    try {
        const response = await axios.delete(`http://localhost:3000/profilpelajar/${deleteId.value}`)
        if (response.status === 200) {
            dialogDelete.value = false
            loadData()
