    ...
    >
      ...
      <template v-slot:item.linkBuku="{ item }">
        <v-btn
          ...
          target="_blank"
          class="rounded-xl"
          prepend-icon="mdi mdi-link-circle-outline"
          size="small"
        >
              ...
              <v-card-text>
                <v-container class="py-4">
                  <v-row>
                    <v-col cols="12">
                      <v-text-field
                        v-model="forms.namaBuku"
                        label="Nama Buku"
                      ></v-text-field>
                    </v-col>
                    <v-col cols="12">
                      <v-text-field
                        v-model="forms.linkBuku"
                        label="Link Buku"
                      ></v-text-field>
                    </v-col>
                  </v-row>
              ...
              <v-card-text>
                <v-container>
                  <v-row>
                    <v-col cols="12">
                      <v-text-field
                        v-model="formsEdit.namaBuku"
                        label="Nama Buku"
                      ></v-text-field>
                    </v-col>
                    <v-col cols="12">
                      <v-text-field
                        v-model="formsEdit.linkBuku"
                        label="Link Buku"
                      ></v-text-field>
                    </v-col>
                  </v-row>
          ...
          <v-dialog v-model="dialogDelete" max-width="500px">
            <v-card class="pa-5 rounded-lg">
              <v-card-title class="text-h5 text-center"
                ...
                Ingin Menghapus Data Buku ini?</v-card-title
              >
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="error" variant="elevated" @click="closeDelete"
                  >Batal</v-btn
                >
                <v-btn
                  color="blue-darken-1"
...
const filteredData = computed(() => {
  ...
  return dataBuku.value.filter((item) =>
    item.namaBuku.toLowerCase().includes(searchData.value.toLowerCase())
  );
});
...
const updateData = async () => {
  try {
    ...
    const response = await axios.put(
      `http://localhost:3000/dbuku/${formsEdit.value.idBuku}`,
      updatedData
    );
    if (response.status === 200) {
...
const deleteItemConfirm = async () => {
  try {
    const response = await axios.delete(
      `http://localhost:3000/dbuku/${deleteId.value}`
    );
    if (response.status === 200) {
